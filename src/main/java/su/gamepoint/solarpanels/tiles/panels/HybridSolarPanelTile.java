 ...
 * @author Dudko Roman
 */
public class HybridSolarPanelTile extends TileEntityGenerator {
    ...
    @Nullable
    protected HybridSolarPanelTile.SolarCheck solarCheck;

    public HybridSolarPanelTile(BlockPos pos, BlockState state) {
        this(BlockRegister.HYBRID_SOLAR_PANEL, pos, state, SolarPanelsMod.getConfig().hybridSolarPanel.get().multiply(2L));
    }

    protected HybridSolarPanelTile(IBlockProvider blockProvider, BlockPos pos, BlockState state, @Nonnull FloatingLong output) {
    ...
    protected void recheckSettings() {
        if (this.level != null) {
            this.solarCheck = new HybridSolarPanelTile.SolarCheck(this.level, this.worldPosition);
    ...
    protected FloatingLong getConfiguredMax() {
        return SolarPanelsMod.getConfig().hybridSolarPanel.get();
    }
