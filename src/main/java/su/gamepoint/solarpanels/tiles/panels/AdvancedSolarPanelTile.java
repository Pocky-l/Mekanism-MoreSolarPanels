 ...
 * @author Dudko Roman
 */
public class AdvancedSolarPanelTile extends TileEntityGenerator {
    ...
    @Nullable
    protected AdvancedSolarPanelTile.SolarCheck solarCheck;

    public AdvancedSolarPanelTile(BlockPos pos, BlockState state) {
        this(BlockRegister.ADVANCED_SOLAR_PANEL, pos, state, SolarPanelsMod.getConfig().advancedSolarPanel.get().multiply(2L));
    }

    protected AdvancedSolarPanelTile(IBlockProvider blockProvider, BlockPos pos, BlockState state, @Nonnull FloatingLong output) {
    ...
    protected void recheckSettings() {
        if (this.level != null) {
            this.solarCheck = new AdvancedSolarPanelTile.SolarCheck(this.level, this.worldPosition);
