 ...
 * @author Dudko Roman
 */
public class QuantumSolarPanelTile extends TileEntityGenerator {
    ...
    @Nullable
    protected QuantumSolarPanelTile.SolarCheck solarCheck;

    public QuantumSolarPanelTile(BlockPos pos, BlockState state) {
        this(BlockRegister.QUANTUM_SOLAR_PANEL, pos, state, SolarPanelsMod.getConfig().quantumSolarPanel.get().multiply(2L));
    }

    protected QuantumSolarPanelTile(IBlockProvider blockProvider, BlockPos pos, BlockState state, @Nonnull FloatingLong output) {
    ...
    protected void recheckSettings() {
        if (this.level != null) {
            this.solarCheck = new QuantumSolarPanelTile.SolarCheck(this.level, this.worldPosition);
