 ...
 * @author Dudko Roman
 */
public class UltimateHybridSolarPanelTile extends TileEntityGenerator {
    ...
    @Nullable
    protected UltimateHybridSolarPanelTile.SolarCheck solarCheck;

    public UltimateHybridSolarPanelTile(BlockPos pos, BlockState state) {
        this(BlockRegister.ULTIMATE_HYBRID_SOLAR_PANEL, pos, state, SolarPanelsMod.getConfig().ultimateHybridSolarPanel.get().multiply(2L));
    }
    ...
    protected void recheckSettings() {
        if (this.level != null) {
            this.solarCheck = new UltimateHybridSolarPanelTile.SolarCheck(this.level, this.worldPosition);
