 ...
 * @author Dudko Roman
 */
public class SpectralSolarPanelTile extends TileEntityGenerator {
    ...
    @Nullable
    protected SpectralSolarPanelTile.SolarCheck solarCheck;

    public SpectralSolarPanelTile(BlockPos pos, BlockState state) {
        this(BlockRegister.SPECTRAL_SOLAR_PANEL, pos, state, SolarPanelsMod.getConfig().spectralSolarPanel.get().multiply(2L));
    }

    protected SpectralSolarPanelTile(IBlockProvider blockProvider, BlockPos pos, BlockState state, @Nonnull FloatingLong output) {
    ...
    protected void recheckSettings() {
        if (this.level != null) {
            this.solarCheck = new SpectralSolarPanelTile.SolarCheck(this.level, this.worldPosition);
    ...
    protected FloatingLong getConfiguredMax() {
        return SolarPanelsMod.getConfig().spectralSolarPanel.get();
    }
