 ...
 * @author Dudko Roman
 */
public class PhotonicSolarPanelTile extends TileEntityGenerator {
    ...
    @Nullable
    protected PhotonicSolarPanelTile.SolarCheck solarCheck;

    public PhotonicSolarPanelTile(BlockPos pos, BlockState state) {
        this(BlockRegister.PHOTONIC_SOLAR_PANEL, pos, state, SolarPanelsMod.getConfig().photonicSolarPanel.get().multiply(2L));
    }

    protected PhotonicSolarPanelTile(IBlockProvider blockProvider, BlockPos pos, BlockState state, @Nonnull FloatingLong output) {
    ...
    protected void recheckSettings() {
        if (this.level != null) {
            this.solarCheck = new PhotonicSolarPanelTile.SolarCheck(this.level, this.worldPosition);
    ...
    protected FloatingLong getConfiguredMax() {
        return SolarPanelsMod.getConfig().photonicSolarPanel.get();
    }
