 ...
 * @author Dudko Roman
 */
public class SingularSolarPanelTile extends TileEntityGenerator {
    ...
    @Nullable
    protected SingularSolarPanelTile.SolarCheck solarCheck;

    public SingularSolarPanelTile(BlockPos pos, BlockState state) {
        this(BlockRegister.SINGULAR_SOLAR_PANEL, pos, state, SolarPanelsMod.getConfig().singularSolarPanel.get().multiply(2L));
    }

    protected SingularSolarPanelTile(IBlockProvider blockProvider, BlockPos pos, BlockState state, @Nonnull FloatingLong output) {
    ...
    protected void recheckSettings() {
        if (this.level != null) {
            this.solarCheck = new SingularSolarPanelTile.SolarCheck(this.level, this.worldPosition);
