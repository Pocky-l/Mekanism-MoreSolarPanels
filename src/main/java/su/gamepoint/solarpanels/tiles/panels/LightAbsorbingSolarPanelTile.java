 ...
 * @author Dudko Roman
 */
public class LightAbsorbingSolarPanelTile extends TileEntityGenerator {
    ...
    @Nullable
    protected LightAbsorbingSolarPanelTile.SolarCheck solarCheck;

    public LightAbsorbingSolarPanelTile(BlockPos pos, BlockState state) {
        this(BlockRegister.LIGHT_ABSORBING_SOLAR_PANEL, pos, state, SolarPanelsMod.getConfig().lightAbsorbingSolarPanel.get().multiply(2L));
    }

    protected LightAbsorbingSolarPanelTile(IBlockProvider blockProvider, BlockPos pos, BlockState state, @Nonnull FloatingLong output) {
    ...
    protected void recheckSettings() {
        if (this.level != null) {
            this.solarCheck = new LightAbsorbingSolarPanelTile.SolarCheck(this.level, this.worldPosition);
    ...
    protected FloatingLong getConfiguredMax() {
        return SolarPanelsMod.getConfig().lightAbsorbingSolarPanel.get();
    }
